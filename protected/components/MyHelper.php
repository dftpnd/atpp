...
class MyHelper
{
    ...
    public static function callbackMakeClickableLinks($match)
    {
        // Prepend http:// if no protocol specified
        $completeUrl = $match[1] ? $match[0] : "http://{$match[0]}";

        return '<a class="classic" target="_blank" href="' . $completeUrl . '">'
    ...
    public static function makeClickableLinks($text)
    {
        ...
        return preg_replace_callback("&\\b$rexProtocol$rexDomain$rexPort$rexPath$rexQuery$rexFragment(?=[?.!,;:\"]?(\s|$))&", 'self::callbackMakeClickableLinks', $text);

    }

    public static function makeClickableLinks2($text)
    {
