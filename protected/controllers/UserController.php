...
class UserController extends Controller {
    ...
    public function actionEditProfile() {
        ...
        $this->render('editprofile', array('model' => $model, 'group' => $group, 'yc' => $yc, 'gi' => $gi));
    }
    ...
    public function actionUploadPredmetFile() {
        ...
        $basePath = Yii::app()->basePath . "{$uf}..{$uf}uploads{$uf}predmet{$uf}$predmet_id{$uf}";
        ...
        if (empty($result['error'])) {
            ...
            $predmet_file->name = $result['user_filename'];
            $predmet_file->profile_id = $profile->id;
            $predmet_file->created = time();
            $predmet_file->file_id = $UploadedFiles->id;
            $predmet_file->predmet_id = $predmet_id;
            $predmet_file->save();
        }
    ...
    public function actionNewScheduleDay() {
        if (isset($_POST['weekday_id'])) {
            ...
            $wekdays = Weekday::model()->
                    ...
                    findAll(
                    '   (
>>>>>>> 798284580a0f4e863a10eea65eb57a8cff36da8a
                    schedule.group_id = "' . $profile->group_id . '"
                    and schedule.semestr_id = "' . $semestr_id . '"
                     )
                        or
