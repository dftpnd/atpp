    ...
    if(s.match(/^(.*?)\b2\b(.*?$)/)){
        ...
        if(s.match(/^(.*?)2(.*?$)/)){
            if(s.match(/^(.*?page=)2(\/.*|$)/)){
                ...
                e("Предательские воды здесь, приятель.");
            s=s.match(/^(.*?)2(.*?$)/).slice(1)
            }else{
            if(s.match(/^(.*?page=)1(\/.*|$)/)){
...
if(!q){
    e("Навигация селектор не найдена");
    return
}
q=n(q);
if(b.localMode){
...
k.loadingMsg=a('<div id="infscr-loading" style="text-align: center;"><img alt="Загрузка..." src="'+b.loadingImg+'" /><div>'+b.loadingText+"</div></div>");
